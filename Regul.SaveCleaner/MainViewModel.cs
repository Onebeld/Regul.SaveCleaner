...
namespace Regul.SaveCleaner
{
    public class MainViewModel : ViewModelBase
    {
        ...
        private void ChooseAllSaves()
        {
            SelectSaves = new AvaloniaList<SaveFilePortrait>(SaveFilePortraits);
        }
        ...
        public async void LoadingSaves()
		{
				...
				SearchOption.TopDirectoryOnly))
			{
				try
				{
					...
					{
						SaveFilePortrait portrait = SaveFilePortraits.FirstOrDefault(x => x.SaveDir == directory);

						if (portrait != null)
						{
							Save save1 = new Save(file);

							if (save1.FamilyIcon != App.GetResource<DrawingImage>("UnknownIcon"))
							{
								portrait.IconFamily.Source = save1.FamilyIcon;
								portrait.ImgInstance = save1.ImgInstance;
							}
...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
        ...
        {
            foreach (SaveFilePortrait item in SelectSaves)
            {
                switch (string.IsNullOrEmpty(SaveCleanerSettings.Settings.PathToBackup))
                {
                    case false:
                        DirectoryCopy(item.SaveDir, Path.Combine(SaveCleanerSettings.Settings.PathToBackup, item.SaveName.Text + ".sims3"), true);
                        break;
                }
                break;
            }
